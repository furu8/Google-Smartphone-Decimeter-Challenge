...
# %%
df = pd.read_csv('../../data/interim/imu_many_lat_lng_deg_lgb3_rf2_rr_.csv')
df
...
# Trueなら下のセル実行OK
try:
    print((sample_df[['collectionName', 'phoneName']].drop_duplicates() == df_mean_pred[['collectionName', 'phoneName']].drop_duplicates()).all().all())
except:
    df_mean_pred = pd.concat([df_mean_pred, df_mean_pred['phone'].str.split('_', expand=True).rename(columns={0:'collectionName', 1:'phoneName'})], axis=1)
    print((sample_df[['collectionName', 'phoneName']].drop_duplicates() == df_mean_pred[['collectionName', 'phoneName']].drop_duplicates()).all().all())

# %%
sample_df['latDeg'] = df_mean_pred['latDeg'].copy()
sample_df['lngDeg'] = df_mean_pred['lngDeg'].copy()
sample_df
# %%
...
test_df = pd.merge_asof(
            ...
            df_mean_pred[['collectionName', 'phoneName', 'millisSinceGpsEpoch', 'latDeg', 'lngDeg']].sort_values('millisSinceGpsEpoch'),
...
df_mon = pd.read_csv('../../data/submission/sample_submission.csv')
df_mon = df_mon.assign(latDeg=sample_df['latDeg'], lngDeg=sample_df['lngDeg'])
df_mon

# %%
# phone_mean
df_mon = pd.concat([df_mon, df_mon['phone'].str.split('_', expand=True).rename(columns={0:'collectionName', 1:'phoneName'})], axis=1)
df_phone_mean = mean_with_other_phones(df_mon)
df_phone_mean
...
visualize_collection(df_mon, cns_dict['SJC'])
...
visualize_collection(df_mon, cns_dict['MTV'])
...
sub
sub.to_csv('../../data/submission/imu_ensemble_kalman_mean_predict_mon_kai_phone_mean.csv', index=False)